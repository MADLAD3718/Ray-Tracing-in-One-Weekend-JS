...
world.hittables.push(new Sphere(new Vec3(0, -1000, 0), undefined, 1000, ground_material));
...
for (let a = -11; a < 11; ++a) for (let b = -11; b < 11; ++b) {
    ...
    if (length(sub(center, new Vec3(4, 0.2, 0))) > 0.9) {
        if (choose_mat < 0.8) {
            ...
            world.hittables.push(new Sphere(center, center1, 0.2, new Lambertian(albedo)));
        } else if (choose_mat < 0.95) {
            const albedo = add(mul(randVec3(), 0.5), new Vec3(0.5, 0.5, 0.5));
            const fuzz = Math.random() * 0.5;
            world.hittables.push(new Sphere(center, undefined, 0.2, new Metal(albedo, fuzz)));
        } else {
            world.hittables.push(new Sphere(center, undefined, 0.2, new Dielectric(1.5)));
        }
    }
}

world.hittables.push(new Sphere(new Vec3(0, 1, 0), undefined, 1.0, new Dielectric(1.5)));
world.hittables.push(new Sphere(new Vec3(-4, 1, 0), undefined, 1.0, new Lambertian(new Vec3(0.4, 0.2, 0.1))));
world.hittables.push(new Sphere(new Vec3(4, 1, 0), undefined, 1.0, new Metal(new Vec3(0.7, 0.6, 0.5), 0)));
